            ...
            try
            {
                  ...
                  if (Minecraft.getMinecraft().thePlayer.getDisplayName() == "maggi373")
                  {
                        MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
                              new ChatComponentText("The pickaxe is at its rightfull owner"));
                  }
                  else
                  {
                        if (tall == 25)
                        {
                              MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
                                    new ChatComponentText("Hey " +
                                                          ...
                                                          ", maggi373 called. He wants his pickaxe back!"));
                        }

                        if (tall == 50)
                        {
                              MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
                                    new ChatComponentText("When diamonds and machines isn't enough"));
                        }

                        if (tall == 75)
                        {
                              MinecraftServer.getServer().getConfigurationManager().sendChatMsg(
                                    new ChatComponentText("Watch out " +
                                                          Minecraft.getMinecraft().thePlayer.getDisplayName() +
                                                          " got maggi373's pickaxe"));
                        }
                  }
            }
            catch (Exception e)
